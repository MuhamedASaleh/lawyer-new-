...
const video = (socket, io) => {
  socket.on('userEnterRoom', async (userId) => {
    if (!clientCaller.userId) {
      clientCaller = { userId, socketID: socket.id, roomId: uuidv4() }; // Generate unique room ID
      io.to(socket.id).emit('userWaiting', `Hello, waiting for another user...`);
    } else {
      clientCallee = { userId, socketID: socket.id, roomId: clientCaller.roomId }; // Use the existing room ID

      // Create a new room and update room ID
      const roomId = clientCaller.roomId; // Use the existing room ID
      if (roomId) {
        // Assuming you have a function to save or update room details in your database
        await createRoom(roomId, clientCaller.socketID, clientCallee.socketID);
  ...
  socket.on('disconnect', async () => {
    if (socket.id === clientCaller.socketID || socket.id === clientCallee.socketID) {
      ...
      if (roomId) {
        await deleteRoomById(roomId); // Clean up the room in your database
